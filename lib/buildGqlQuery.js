...
export const buildFields = introspectionResults => parentType => parentType.fields.reduce((acc, field) => {
  ...
  if (linkedType) {
    ...
    return _extends({}, acc, {
      [field.name]: {
        fields: buildFields(introspectionResults)(linkedType)
      }
    });
  }
...
export default (introspectionResults => (resource, aorFetchType, queryType, variables) => {
  ...
  if (aorFetchType === UPDATE) {
    return encodeMutation(queryType.name, {
      params: _extends({}, apolloArgs, { "$id": "ID!" }),
      fields: {
        data: {
          field: queryType.name,
