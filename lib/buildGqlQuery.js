...
export const buildFields = introspectionResults => fields => fields.reduce((acc, field) => {
    ...
    if (type.kind !== TypeKind.OBJECT) {
        return _extends({}, acc, { [field.name]: {} });
    }
...
export const buildArgs = (query, variables, inputType) => {
    ...
    let args = inputType.inputFields.filter(a => validVariables.includes(a.name)).reduce((acc, arg) => _extends({}, acc, { [`${arg.name}`]: `$${arg.name}` }), {});
...
export const buildApolloArgs = (query, variables, inputType) => {
    ...
    let args = inputType.inputFields.filter(a => validVariables.includes(a.name)).reduce((acc, arg) => {
