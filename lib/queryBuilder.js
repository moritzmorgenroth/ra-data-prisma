...
export const buildArgs = (query, variables) => {
    ...
    let args = query.args.filter(a => validVariables.includes(a.name)).reduce((acc, arg) => _extends({}, acc, { [`${arg.name}`]: `$${arg.name}` }), {});
...
export const buildApolloArgs = (query, variables) => {
    ...
    let args = query.args.filter(a => validVariables.includes(a.name)).reduce((acc, arg) => {
        if (arg.name.endsWith('Ids')) {
            return _extends({}, acc, { [`$${arg.name}`]: '[ID!]' });
        }
...
export const sanitizeResource = (introspectionResults, resource) => data => {
    const result = Object.keys(data).reduce((acc, key) => {
        ...
        if (type.kind !== TypeKind.OBJECT) {
            return _extends({}, acc, { [field.name]: data[field.name] });
        }
