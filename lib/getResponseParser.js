    ...
    const result = Object.keys(data).reduce((acc, key) => {
        ...
        if (linkedResource) {
            ...
            if (Array.isArray(linkedResourceData)) {
                ...
                return _extends({}, acc, {
                    [field.name]: data[field.name].map(sanitizeResource(introspectionResults, linkedResource)).map(d => d.id)
                });
            }

            return _extends({}, acc, {
                [`${field.name}.id`]: linkedResourceData ? data[field.name].id : undefined,
                [field.name]: linkedResourceData ? sanitizeResource(introspectionResults, linkedResource)(data[field.name]) : undefined
            });
        }
